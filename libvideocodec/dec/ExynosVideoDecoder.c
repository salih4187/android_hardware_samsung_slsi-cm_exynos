    ...
    ExynosVideoFramePacking *pFramePacking)
{
    ...
    int seiAvailable, seiInfo, seiGridPos, i __unused;
 ...
 */
static ExynosVideoErrorType __unused MFC_Decoder_Get_Buffer_Inbuf(
    void               *pHandle,
    int                 nIndex,
 ...
 */
static ExynosVideoErrorType __unused MFC_Decoder_Wait_Inbuf(void *pHandle)
{
    ExynosVideoDecContext *pCtx = (ExynosVideoDecContext *)pHandle;
    ExynosVideoErrorType   ret  = VIDEO_ERROR_NONE;
    ...
    for (nIndex = 0; nIndex < pCtx->nInbufs; nIndex++) {
        if (pCtx->pInbuf[nIndex].bRegistered == VIDEO_FALSE) {
            for (plane = 0; plane < nPlanes; plane++) {
                ...
                ALOGV("%s: registered buf %d (addr=%p alloc_sz=%x fd=%d)\n", __func__, nIndex,
